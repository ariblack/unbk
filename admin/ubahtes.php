							  <?php
								...
								$sqlujian = mysqli_query($sqlconn_i, "select * from cbt_ujian where XKodeSoal = '$_REQUEST[txt_ujian]'");
								...
								$jumsoal = mysqli_num_rows(mysqli_query($sqlconn_i, "select * from cbt_soal where  XKodeSoal = '$_REQUEST[txt_ujian]'"));
								$val_banksoal =  "$jumsoal";
								$val_jumsoal = str_replace(" ", "", $_REQUEST['txt_soal']);

								if ($val_jumsoal > $val_banksoal) {
								...
								//  $sqlubah = mysqli_query("insert into cbt_sampah (anu) values ('$_REQUEST[txt_ujian]')");
								$sqlinsert = mysqli_query($sqlconn_i, "insert into cbt_ujian 
							  (XKodeKelas,XKodeMapel,XTokenUjian,XTglUjian,XJamUjian,XLamaUjian,XBatasMasuk,XJumSoal,XKodeSoal,XStatusUjian,XSisaWaktu,XNilai)
							  values 
							  ('$_REQUEST[txt_level]','$_REQUEST[txt_mapel]','$_REQUEST[txt_token]','$tgl','$jam','$jame','$telatujian','$ambilsoal',
							  '$_REQUEST[txt_ujian]','1','$telatujian','0')");

								/*$sqlubah2 = mysqli_query("update cbt_ujian set XStatusUjian = '1',XTokenUjian = '$_REQUEST[txt_token]',
