...
//$xkodesoal = "XGAL1SOAL1";
  ...
  $s0 = mysqli_fetch_array($sqluser);
  $xkodesoal = $s0['XKodeSoal'];
  $xtokenujian = $s0['XTokenUjian'];  


$sql = mysqli_query($sqlconn_i, "SELECT j.XNomerSoal as XNomerSoal, j.XJawaban as XJawaban , c.XNomerSoal as XNomerSoal, j.XRagu as XRagu
FROM  `cbt_soal` c
LEFT JOIN cbt_jawaban j ON ( j.XNomerSoal = c.XNomerSoal
AND j.XKodeSoal = c.XKodeSoal ) where c.XKodeSoal = '$xkodesoal' and j.XUserJawab = '$user' and j.XTokenUjian = '$xtokenujian' order by j.XNomerSoal");
...
?>        
            ...
            <?php echo '<a href="#" data-id="'.$s['XNomerSoal'].'" class="get_pic">'; ?>
           <div class="item" style="background-color:<?php echo $cssb; ?>; color:<?php echo $csst; ?>;border-color:<?php echo $border; ?>"><p style="margin-top:-9px; margin-left:-9px; font-family:Arial, Helvetica, sans-serif; font-size:24px">
		   <?php // echo "$iki|$_REQUEST[pic]|$s[XNomerSoal]|"; ?>
		   <?php // echo $s['XNomerSoal']; 
		   echo "$s[XNomerSoal]";
		   ?></p>
           <div  id='noti-count' style="border-color:<?php echo $border; ?>"><div><?php echo $s['XJawaban']; ?></div></div></div></a>
           <?php } ?>         
        </div>
		...
		document.getElementById("D").checked = true;		
				...
				if(e.which==65||e.which==66||e.which==67||e.which==68||e.which==69) {
				  ...
				  $("#tkn").html(tekan+' '+soale);
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "simpan.php?kode=<?php echo $xkodesoal; ?>",
                        data: data,
                        success: function() {
...
//Connect to Database
//$sqlj = mysqli_query("SELECT * from cbt_jawaban where XNomerSoal = '$_POST[pic]' and XKodeSoal = '$xkodesoal' and XUserJawab = '$user'");
$sqlj = mysqli_query($sqlconn_i, "
SELECT * 
FROM cbt_soal s
LEFT JOIN cbt_jawaban j ON (j.XNomerSoal = s.XNomerSoal and j.XKodeSoal = s.XKodeSoal)
WHERE j.XNomerSoal =  '$_POST[pic]'
...
//get next picture id
//$sql = mysqli_query("SELECT id FROM pictures WHERE id > '$current_picture' ORDER BY id ASC LIMIT 1");
//$sql = mysqli_query("SELECT XNomerSoal FROM cbt_soal WHERE XNomerSoal > '$current_picture' and XKodeSoal = '$xkodesoal' ORDER BY XNomerSoal ASC LIMIT 1");

$sql = mysqli_query($sqlconn_i, "SELECT XNomerSoal FROM cbt_jawaban WHERE XNomerSoal > '$current_picture' and XKodeSoal = '$xkodesoal'
and XUserJawab = '$user' and XTokenUjian = '$xtokenujian' ORDER BY XNomerSoal ASC LIMIT 1");
...
//$sql = mysqli_query("SELECT XNomerSoal FROM cbt_soal WHERE XNomerSoal < $current_picture and XKodeSoal = '$xkodesoal' ORDER BY XNomerSoal DESC LIMIT 1");

$sql = mysqli_query($sqlconn_i, "SELECT XNomerSoal FROM cbt_jawaban WHERE XNomerSoal < '$current_picture' and XKodeSoal = '$xkodesoal'
and XUserJawab = '$user' and XTokenUjian = '$xtokenujian' ORDER BY XNomerSoal DESC LIMIT 1");
$result = mysqli_fetch_array($sql);
if($result){
...
//$sql = mysqli_query("SELECT * FROM cbt_soal WHERE XNomerSoal = $current_picture and XKodeSoal = '$xkodesoal' LIMIT 1");
$sql = mysqli_query($sqlconn_i, "SELECT * 
FROM cbt_soal s
LEFT JOIN cbt_jawaban j ON (j.XNomerSoal = s.XNomerSoal and j.XKodeSoal = s.XKodeSoal)
WHERE j.XNomerSoal =  '$_POST[pic]'
AND s.XKodeSoal =   '$xkodesoal'
...
<!-- Audio Biasa 	
  ...
  <source src="<?php echo "$audfile"; ?>" type="audio/mpeg">
  <source src="<?php echo "$audfile"; ?>" type="audio/ogg">
  <embed height="90" width="100"  src="<?php echo "$audfile"; ?>">
  </embed>
  </audio>
  </div>
 !-->
...
<?php if(str_replace(" ","",$result['XVideoTanya'])!==''){
...
<div class="play"> 
 <video width="400" controls>
  <source src="<?php echo "$vidfile"; ?>" type="video/mp4">
  <source src="<?php echo "$vidfile"; ?>" type="video/ogg">
...
<!--		
		<input id="soale" type="hidden" name="soale" value="<?php echo $current_picture; ?>"/>
		<table border="0" cellpadding="0px" cellspacing="0px">
		<tr><td valign="top"><input id="A" type="radio" name="credit-card" value="A" <?php echo $nilaiA; ?>>
        <label class="drinkcard-cc A" for="A">&nbsp;</label></td><td class="pilihanjawaban"><?php echo "$result[XJawab1]"; ?></td></tr>
        <br /><input id="B" type="radio" name="credit-card" value="B" onclick='alertt(this.value)'/>
        <label class="drinkcard-cc B"for="B">&nbsp;</label><p class="jawaban"><?php echo "$result[XJawab2]"; ?></p> 
	...
	echo "<img src='pictures/$result[XGambarJawab2]'><br /><br />";} ?>
        <?php echo "$result[XJawab2]"; ?></td></tr>
  		<tr><td valign="top"><input id="C" type="radio" name="credit-card" value="C" <?php echo $nilaiC; ?>>
        <label class="drinkcard-cc C" for="C">&nbsp;</label></td><td class="pilihanjawaban"><?php echo "$result[XJawab3]"; ?></td></tr>
        <tr><td valign="top"><input id="D" type="radio" name="credit-card" value="D" <?php echo $nilaiD; ?>>
        <label class="drinkcard-cc D" for="D">&nbsp;</label></td><td class="pilihanjawaban"><?php echo "$result[XJawab4]"; ?></td></tr>
  		
        </table>
!-->
<?php $ts=$sj['XTypeSoal']; if($ts=='0'){
	echo "<input id='soale' type='hidden' name='soale' value='$current_picture'/>";
	echo "<table border='0' cellpadding='0px' cellspacing='0px'>
		<tr><td valign='top'><input id='A' type='radio' name='credit-card' value='A' $nilaiA >
        <label class='drinkcard-cc A' for='A'>&nbsp;</label></td><td class='pilihanjawaban'>$jwbA </td></tr>
       	<tr><td valign='top'><input id='B' type='radio' name='credit-card' value='B' $nilaiB >
        <label class='drinkcard-cc B' for='B'>&nbsp;</label></td><td class='pilihanjawaban'>$jwbB </td></tr>
  		<tr><td valign='top'><input id='C' type='radio' name='credit-card' value='C' $nilaiC >
        <label class='drinkcard-cc C' for='C'>&nbsp;</label></td><td class='pilihanjawaban'>$jwbC </td></tr>
        <tr><td valign='top'><input id='D' type='radio' name='credit-card' value='D' $nilaiD >
        <label class='drinkcard-cc D' for='D'>&nbsp;</label></td><td class='pilihanjawaban'>$jwbD </td></tr>
  		</table>";
        
        	}
			else {$se=$sj['XEssay'];echo "	<form method='post' class='form-user'>	<input type='hidden' name='o' value='O'/>	<input id='soale' type='hidden' name='soale' value='$current_picture'/>"; echo "<strong>Jawaban :<br>
<div class='tampildata'></div></strong><br><textarea  cols='80' rows='6' name='esse' >$se</textarea><br><a class='tombol-simpan'>simpan</a></form>"; }
...
// $cek = mysqli_query("select * from cbt_jawaban where XNomerSoal='$stu' and XKodeSoal ='$xkodesoal'"); 
 $cek = mysqli_query($sqlconn_i, "select * from cbt_jawaban where XNomerSoal='$stu' and XKodeSoal ='$xkodesoal' and XTokenUjian = '$xtokenujian'"); 
