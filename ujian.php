...
$sqluser = mysqli_query($sqlconn_i, "SELECT * , u.XKodeKelas AS kelaz, u.XKodeSoal AS soalz FROM  `cbt_siswa` s LEFT JOIN cbt_ujian u ON s.XKodeKelas = u.XKodeKelas 
...
$mulai = $s2['XMulaiUjian'];
$xlast = $s2['XLastUpdate'];
$lama = $s2['XLamaUjian'];
...
if ($jumsqlceksiswa < 1) { // jika siswa belum pernah login 
    ...
    if ($xjam1 > $xbatasmasuk) {
        $sqlout = mysqli_query($sqlconn_i, "Update cbt_siswa_ujian set XStatusUjian = '9' where XNomerUjian = '$user' and XStatusUjian = '1' and XTokenUjian ='$xtokenujian'");
        header('location:logout.php');
    }
    ...
    $sqlinputsiswa = mysqli_query($sqlconn_i, "insert into cbt_siswa_ujian 
    ...
    $hasil = strtotime($xlast) - strtotime($mulai);
    $jamkonvert = explode(":", $lama);
    $detik = $jamkonvert[0] * 3600 + $jamkonvert[1] * 60 + $jamkonvert[2];
    $waktu = $detik - $hasil;

    function konversi_detik($waktu)
    {
        $dtka = $waktu % 60;
        $mnta = floor(($waktu % 3600) / 60);
        $jama = floor(($waktu % 86400) / 3600);

        $d3a = sprintf("%02d", $dtka);
        $m3a = sprintf("%02d", $mnta);
        $j3a = sprintf("%02d", $jama);

        return $j3a . ':' . $m3a . ':' . $d3a;
    };

    $waktua = konversi_detik($waktu);
    $sqlwaktu = mysqli_query($sqlconn_i, "Update cbt_siswa_ujian set XMulaiUjian = '$xjam1',XLastUpdate = '$xjam1', XLamaUjian = '$waktua' where XNomerUjian = '$user' and XStatusUjian = '1' and XTokenUjian ='$xtokenujian'");
...
<head>
    ...
    <?php
    $sql1 = mysqli_query($sqlconn_i, "select * from cbt_soal where XKodeSoal = '$xkodesoal' ");
    while ($ambil1 = mysqli_fetch_array($sql1)) {
        $Xacak = $ambil1['Xidacak'];
        $XTypeSoal = $ambil1['XTypeSoal'];
        $XPilSoal = $ambil1['XPilSoal'];
        if ($XPilSoal == 4) {
            if ($Xacak == '1') {
                $cek = mysqli_num_rows(mysqli_query($sqlconn_i, "select * from cbt_jawaban where XKodeSoal = '$xkodesoal' and XUserJawab = '$user' and XTokenUjian = '$xtokenujian'"));
                if ($cek < 1) {
                    $hit = 1;
                    $sqlambilsoal = mysqli_query($sqlconn_i, "select * from cbt_soal where XKodeSoal = '$xkodesoal'  order by RAND() LIMIT $xjumlahsoal");
                    while ($ambilsoal = mysqli_fetch_array($sqlambilsoal)) {
                        ...
                        $sql = mysqli_query($sqlconn_i, "insert into cbt_jawaban (Urut,XNomerSoal,XUserJawab,XKodeSoal,XTokenUjian,XKunciJawaban,XA,XB,XC,XD,XTglJawab) values ('$hit','$ambilsoal[XNomerSoal]','$user','$xkodesoal','$xtokenujian','$ambilsoal[XKunciJawaban]','$A1','$B1','$C1','$D1','$tglbuat')");
                        $hit = $hit + 1;
                    }
                }
            } else {
                ...
                if ($cek < 1) {
                    $hit = 1;
                    $sqlambilsoal = mysqli_query($sqlconn_i, "select * from cbt_soal where XKodeSoal = '$xkodesoal' order by XNomerSoal ASC LIMIT $xjumlahsoal");
                    while ($ambilsoal = mysqli_fetch_array($sqlambilsoal)) {
            ...
            if ($Xacak == '1') {
                ...
                if ($cek < 1) {
                    ...
                    while ($ambilsoal = mysqli_fetch_array($sqlambilsoal)) {
                        ...
                        $sql = mysqli_query($sqlconn_i, "insert into cbt_jawaban (Urut,XNomerSoal,XUserJawab,XKodeSoal,XTokenUjian,XKunciJawaban,XA,XB,XC,XD,XE,XTglJawab) values ('$hit','$ambilsoal[XNomerSoal]','$user','$xkodesoal','$xtokenujian','$ambilsoal[XKunciJawaban]','$A1','$B1','$C1','$D1','$E1','$tglbuat')");
                        $hit = $hit + 1;
                    }
                }
            } else {
...
<body>
    <main>
                    ...
                    <div class="pull-right bg-dark">
                        <div class="access-panel">
                            ...
                            <div class="ac-info">
                                ...
                                <span class="ac-name"><?php echo "$val_siswa ($user)"; ?></span>
        ...
        <?php
        isset($_GET['cssb']) ? $cssb = $_GET['cssb'] : $cssb = '#336898';
        ...
        <?php
        $sql12 = mysqli_query($sqlconn_i, "select distinct XPilSoal from cbt_soal where XKodeSoal = '$xkodesoal' ");
        while ($ambil12 = mysqli_fetch_array($sql12)) {
            $XPilSoal2 = $ambil12['XPilSoal'];
            if ($XPilSoal2 == 4) { ?>

                <script type="text/javascript">
                    $(document).ready(function() {

                        $.post("getsoal4.php?kode=<?php echo $xkodesoal; ?>", {
                            pic: "1"
                        }, function(data) {
                            $("#picture").html(data);
                            $("#soal").html(1);
            ...
            <?php
                ...
                <script type="text/javascript">
                    $(document).ready(function() {
                        ...
                        $.post("getsoal.php?kode=<?php echo $xkodesoal; ?>", {
                            pic: "1"
                        }, function(data) {
                            $("#picture").html(data);
                            $("#soal").html(1);
